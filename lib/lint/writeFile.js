...
const _writeGitIgnore = () => {
  fs.access(process.cwd(), '.gitignore', (isNone) => {
    if (isNone) {
      fs.writeFileSync(path.join(process.cwd(), '.gitignore'), fs.readFileSync(path.join(CONFIG_PATH, '.gitignore')));
      console.log(chalk.green('.gitignore文件'));
    }
  });
};
